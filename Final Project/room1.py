...
if face_cascade.empty():
	raise IOError("Unable to load the load the face cascade classifier XML file")

cap = cv2.VideoCapture(0)
scaling_factor = 0.5

pickle_files = glo.glob("Faces/*.pickle")
...
for data in face_data:
	...
	if "name" in data and "face_encoding" in data and "access" in data:
		...
		known_face_access.append([access.lower() for access in data['aceess']])

while True:
	
	_, frame = cap.read()
	frame = cv2.resize(frame, None, fx = scaling_factory, fy = scaling_factory, interpolation = cv2.INTER_AREA)
	...
	for (x,y, w,h) in face_rects:
		...
		matched_names = []
		distances = face_recognition.facedistance(known_face_encodings, face_encoding)
